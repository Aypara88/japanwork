...
export default function App() {
  ...
  useEffect(() => {
    const update = () => {
      ...
      const spScale = vw / SP_DESIGN_WIDTH;
      // Cap PC scale at 1 — no upscaling beyond native 1440px
      const pcScale = Math.min(1, vw / PC_DESIGN_WIDTH);
      setState({ spScale, pcScale, isMobile, ready: true });
    };
    update();
  ...
  return (
    <div className="bg-white w-full overflow-x-hidden">
      ...
      {!state.isMobile && (() => {
        const vw = typeof window !== "undefined" ? window.innerWidth : PC_DESIGN_WIDTH;
        // When viewport > 1440px, center the 1440px design; otherwise left-align and scale
        const leftOffset = Math.max(0, (vw - PC_DESIGN_WIDTH) / 2);
        return (
          <div style={{ width: "100vw", height: PC_DESIGN_HEIGHT * state.pcScale, position: "relative", overflow: "hidden", backgroundColor: "#fff" }}>
            <div style={{ width: PC_DESIGN_WIDTH, height: PC_DESIGN_HEIGHT, position: "absolute", top: 0, left: leftOffset, transformOrigin: "top left", transform: `scale(${state.pcScale})` }}>
              <PcLP />
            </div>
          </div>
        );
